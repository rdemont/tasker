...
class TagsWidget extends StatefulWidget {
  ...
  TagsWidgetState createState() {
    return TagsWidgetState();
  }
}

class TagsWidgetState extends State<TagsWidget> {
  final FocusNode _chipFocusNode = FocusNode();
  //late List<Tag> _toppings ;
  late List<Tag> _suggestions = <Tag>[] ; 

    @override
  void initState() {
    //_toppings = widget.selectedTag ; 
print("ALL TAGS  ${widget.allTags.length} - SELECTED TAG : ${widget.selectedTag.length}");    
    super.initState();
  }
...
print("_onSearchChanged*************");
    final List<Tag> results = await _suggestionCallback(value);
print("_onSearchChanged************* NB: ${results.length}");    
    setState(() {
      _suggestions = results
          .where((Tag topping) => !widget.selectedTag.contains(topping))
          .toList();
    });
  }

  Widget _chipBuilder(BuildContext context, Tag topping) {
print("_chipBuilder") ;    
//return Text("ERRRORRR");
  ...
  void _selectSuggestion(Tag topping) {
print("_selectSuggestion") ;        
    setState(() {
      widget.selectedTag.add(topping);
      _suggestions = <Tag>[];
    });
  }

  void _onChipTapped(Tag topping) {
print("_onChipTapped") ;                
  }

  void _onChipDeleted(Tag topping) {
print("_onChipDeleted") ;            
    setState(() {
      widget.selectedTag.remove(topping);
      _suggestions = <Tag>[];
    });
  }

  void _onSubmitted(String text) {
print("_onSubmitted -- Add new Tag ***************** NB: ${widget.selectedTag.length}") ;    
    Tag tag = TagGen.newObj();
    tag.name = text.trim() ;
    ...
    if (text.trim().isNotEmpty) {
      ...
      //  widget.selectedTag = <Tag>[...widget.selectedTag,  tag];
      });
print("++_onSubmitted -- Add new Tag ***************** NB: ${widget.selectedTag.length}") ;          
    } else {
  ...
  FutureOr<List<Tag>> _suggestionCallback(String text) {
    if (text.isNotEmpty) {
      return widget.allTags.where((element) => element.name.toLowerCase().contains(text.toLowerCase())).toList();
    }
    return const <Tag>[];
  }
}
...
print("ToppingInputChip build");
    return Container(
      ...
      child: InputChip(
        ...
        avatar: CircleAvatar(
          child: Text(topping.name[0].toUpperCase()),
        ),
